...
### 8.3 报表型页面（`/store-pnl`、`/financial-model`，2026-08-20）

分析页是「卡片 + 图表」，报表页是「行 × 列的表」。骨架仍照 §8.2（PageHeader → 筛选条 → DataTrustBar → 主体），但主体换成表格，另加五条约束：

- **口径不混算。** 经营口径与 IFRS 16 口径 side-by-side 时，**basis 标签上在块级**（区块头），不是逐行小标。两个口径之间不得出现合计行——这是 §2 原则 7 在报表版面上的落点，比脚注更硬。
- **缺失是 dash，不是 0。** 引擎给的是具名 Gap，版面上渲染为 `—` 并可 hover 出原因。**任何位置都不许把缺失显示成 0**，那会让读者以为「这项是零」而不是「这项没有数」。
- **未经治理的自定义公式行必须带标识**，且标识随数字走——JSON、XLSX、页面三处都要有。只在页面上标，导出后就没人知道了。
- **多币种分区呈现，不给跨币种合计。** 分区之间留视觉间隔，不要做成看起来能相加的连续表格。
- **前端不重算任何模型行。** 公式行与小计行由后端同一 AST 求值后下发；前端只渲染。想在前端加个「顺手算一下」的小计，就是在制造第二套数字。

行操作（显隐、分组折叠）与保存视图属于**视图状态**，不改变任何数值，不得与筛选条的取数参数混在一处控件里。
