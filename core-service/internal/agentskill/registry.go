...
func ProductionRegistry() *Registry {
	...
	for _, definition := range []Definition{
		...
		},
		{
			...
			AllowedTools:  []string{"lease.portfolio.summary", "lease.management.pre_read", "lease.store.performance", "lease.rent_to_sales", "lease.equipment.performance", "lease.fpna.actions", "lease.close.readiness", "lease.budget.variance", "lease.cashflow.scenario", "lease.renewal.decisions", "lease.store.scenario.simulate", "lease.equipment.scenario.simulate", "lease.deal.simulate", "lease.predeal.simulate", "lease.renewal.simulate", "lease.decision.summary", "lease.fpna.action.draft.create", "lease.explanation.draft.create", "lease.meeting.action.draft.create", "lease.decision.memo.draft.create", "lease.fpna.scenario.draft.create", "lease.contract.search", "fpna.store_pnl.read", "lease.monthly_closing.batches.read", "lease.monthly_closing.entries.preview", "lease.monthly_closing.periods.read", "lease.monthly_closing.lock_status.read", "retail.operating_facts.stores.read", "retail.operating_facts.store_days.read", "retail.kpis.store_days.read", "lease.report.schedule.read", "lease.report.disclosure_package.read", "lease.report.contract_view.read", "lease.report.unit_price.read", "lease.report.tags.read"},
