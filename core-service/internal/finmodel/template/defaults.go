...
func DefaultStorePnlTemplate() (*Template, error) {
	return Parse(TemplateDef{
		...
		Rows: []RowDef{
			...
			{Key: "gross_profit_line", Label: "毛利（合计）", Kind: RowSubtotal, Basis: BasisShared, Children: []string{"revenue", "cogs"}, Subtract: []string{"cogs"}},
			...
			{Key: "occupancy_cost", Label: "经营占用成本（Occupancy Cost）", Kind: RowSubtotal, Basis: BasisOperating, Children: []string{"fixed_rent", "service_fee", "variable_rent"}},
			{Key: "store_contribution", Label: "门店贡献（Store Contribution）", Kind: RowSubtotal, Basis: BasisOperating, Children: []string{"gross_profit", "labor_cost", "marketing", "non_lease_cost", "other_controllable", "occupancy_cost"}, Subtract: []string{"labor_cost", "marketing", "non_lease_cost", "other_controllable", "occupancy_cost"}},
...
func statementRows() []RowDef {
	return []RowDef{
		...
		{Key: "opex", Label: "经营费用合计", Kind: RowSubtotal, Basis: BasisOperating, Children: []string{"labor", "fixed_rent", "variable_rent", "non_lease", "other_ctrl"}},
		...
		{Key: "total_assets", Label: "资产合计", Kind: RowSubtotal, Basis: BasisShared, Children: []string{"cash", "ar", "inventory", "ppe", "rou_asset"}},
		{Key: "total_liabilities", Label: "负债合计", Kind: RowSubtotal, Basis: BasisShared, Children: []string{"ap", "lease_liability", "borrowings"}},
		...
		{Key: "total_equity", Label: "所有者权益合计", Kind: RowSubtotal, Basis: BasisShared, Children: []string{"share_capital", "retained_earnings"}},
		// — 现金流量表 CF（间接法）—
