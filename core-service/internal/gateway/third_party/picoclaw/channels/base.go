...
// Upstream path: channels/base.go
...
//   - BaseChannel.bus field changed from *bus.MessageBus (concrete) to the local
//   - bus.MessageBus interface; the runner supplies a one-method implementation.
...
import (
	...
	"github.com/lease-management-system/core-service/internal/gateway/third_party/picoclaw/identity"
	"github.com/lease-management-system/core-service/internal/gateway/third_party/picoclaw/logger"
	"github.com/lease-management-system/core-service/internal/gateway/third_party/picoclaw/media"
)

// MessageBus is the local alias of the vendored bus interface. Upstream
// holds a concrete *bus.MessageBus; this tree narrows it to the one method
// the channel layer uses (see file header, local adaptations).
