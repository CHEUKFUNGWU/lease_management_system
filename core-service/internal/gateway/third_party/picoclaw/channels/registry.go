...
// Upstream path: channels/registry.go
...
type ChannelFactory func(channelName, channelType string, cfg *config.Config, bus MessageBus) (Channel, error)
...
func RegisterSafeFactory[S any](
	channelType string,
	ctor func(bc *config.Channel, settings *S, bus MessageBus) (Channel, error),
) {
	RegisterFactory(channelType, func(channelName, _ string, cfg *config.Config, b MessageBus) (Channel, error) {
...
// FactoryFor looks up a channel factory by name; nil when absent.
func FactoryFor(name string) ChannelFactory {
