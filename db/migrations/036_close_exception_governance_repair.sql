...
--
-- Migration 019 is included in db/init/01_init.sql for fresh databases, but
-- PostgreSQL only runs that file when the data volume is first created. Some
-- existing development/production volumes therefore have the close-control
-- code deployed without its tables. Keep this migration idempotent so it can
-- be applied safely to those volumes without rebuilding business data.
