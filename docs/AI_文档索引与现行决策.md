...
| [Spec：Agent Runtime 完整升级（C1 批次）](specs/agent-runtime-overhaul-c1.md) | **Current** | **C1 批次口径来源**：D-C0~D-C10，Story 1–43。范围按能力清单判不按包判（D32）；三层推进各自可独立回退。**进度（2026-08-26）**：前两层完成并验收；第三层六项按 D44 裁剪为三项，三项均已交付并复验，AR4 / 模型路由 / AR6 推迟。**尚未完成 = 不得写成「C1 完成」** |
| [工单：C1 第三层（L3）](specs/agent-runtime-c1-layer3.md) | **Current（收尾中）** | 四部分（三个尾巴 / 健康检查 / 定时触发 / MCP）全部交付并复验；唯一未完项是 A-3 观察窗口出数据小结。数据到手后本工单即完结并按 §1.5 惯例删除 |
| [CodebaseDesign：Agent Runtime 升级模块深化](CodebaseDesign_AgentRuntime升级_模块深化.md) | **Current** | **C1 实施级设计**：AR1 ContextKey（已交付）、AR2 Session Manager（已交付）、AR3 上下文工程（已交付并接线，flag 默认关，见 D38）、AR4 Subturn 委派（**按 D44 推迟**，接口设计保留）、AR5 汇流与治理链上生产（b/c/d 已交付；「内核置换」口径订正见 ADR-0028 Correction）、AR6 Memory（**按 D44 推迟**，`ContextKey` 五维隔离键与 `narrow` 求交设计保留）；决策留痕 D-C11~D-C20 与守卫 AR*-G* |
...
**2026-08-26 的收缩（C1 批次工单）**：删除五份已完结的 C1 交付工单——`agent-runtime-c1-remaining-work.md`（RT1）、`agent-runtime-c1-review-fixes.md`（AF1–AF4）、`agent-tool-rejection-code-fidelity.md`（RC1）、`session-cross-entity-isolation-and-ar2-wiring.md`（SI1）、`agent-run-read-path-entity-boundary.md`（SI2），共 496 行。**沿用 2026-08-20 的先例，不进归档区**：它们是流程产物不是推理产物，结论已被现行文档吸收，留着只增加误读面积（五份的状态行还都停在 `ready-for-agent`，本身就是误读源）。

**删除前逐份核过「结论已吸收」这个前提**，三份不满足的当场补登记：RC1 → **D41**（`scope_denied` 是底线 1 专属证据码，不得被其他拒绝伪装；guard 与非 guard 两条路径同场景同码）；SI1+SI2 → **D42**（法人边界必须落在仓储方法签名上，会话闸门保护不了不加载会话的 handler）；AF1-a → **D43**（实测与估算不得合并成一个数；测试输入语义必须与生产写入语义一致）。RT1 的结论此前已散在 D38 / G9 / §5 #14 中，本轮随 #14 更新一并收口。代码留痕见各条 D 的第三列。
...
| **D39** | **定时任务两类分治 + Type A 跨法人豁免登记（RT1-L3-C，2026-08-26）。** 调度任务分两类：**Type A 系统维护型**（lease recovery、capability cleanup、auth-refresh cleanup）不产生 run、不调工具，故不进治理链——九控制全是围绕 ToolCall 的闸门，给不调工具的运维动作造假 ToolCall 过链本身是新的说谎面；授权形态为部署级机器信任（同 worker 轴 #14 的信任模型）。**跨法人是 Type A 的定义本身**：只重置 status/lease 列、不读任何租户业务字段；**边界：一旦此类 job 需要读业务字段，本条豁免即失效**，须重新走 Principal 裁决。**Type B 业务运行型**（Story 29 等）产生真实 run 入队，必须走治理链 + 幂等键（job_id+触发时刻派生）+ 登记表 Principal（scope 不宽于登记者，D-C17 narrow 思路）；登记表来源（表 vs 配置）与未决 #14 同场裁决。结构性守卫：internal/scheduler 仅依赖标准库（import guard 双向测试含种违规 fixture），Registration.Run 签名无任何 runtime/registry 参数 | L3 工单 L3-C；`tmp/delivery-L3-C.md` |
| **D40** | **MCP 接入的边界与底线登记（RT1-L3-D，2026-08-26）。** ① 信任边界：工具治理属性（level/read_only/权限/超时）只来自 first-party 登记清单，server 自报元数据不参与治理判定（无合并代码路径，结构保证）；清单 input_schema 是唯一允许出站的字段集合，handler 按 schema 重建出站载荷（未声明字段丢弃 = 白名单边界），键名/标识值扫描是纵深防御不是边界。键名/标识值扫描是纵深防御不是边界（扫描覆盖数组内裸字符串值）；**白名单默认拒绝未知形状**：只认已声明的标量 / array（整体透传不做项内投影，v1 登记简化）/ object（全深度按声明投影；无 sub-properties 投影成 `{}`），**无 type、未知 type 值、带子声明无 type 一律 egress 报错拒绝 + Validate 登记期同规则拒绝**——「没写 type」不是放行路径。② 子进程环境：cmd.Env 显式白名单（PATH + 清单声明），绝不 nil——nil 会把 DB_PASSWORD/JWT_SECRET/MinIO root/LLM key 交给外部进程（变异测试钉住）。②' client 层并发与生命周期（复验两轮返工）：每 server 一个长命 client 被其全部工具共享，单读取循环 + pending map 按 id 分发，调用级并发安全；读路径：单行硬上限 1 MiB，超限传输层不可信 → 毒化 + 组杀；写路径：cap-1 写槽（获取即 select ctx）+ stdin fd 设 caller deadline，wedged server 不再能永久挂住调用者，**部分写弃置 = 帧边界丢失 → 同样毒化**（n=0 失败不毒化）；握手 5s / 目录 10s 上界（不应答 → 拒绝启动，非挂死）；server 主动请求（method+id）不消费我们的 pending id；Close 对进程组二次扫描（shell 被 kill 时 fork 中的孙进程可错过首波组信号）。③ **底线 2 登记（本版不按分类区分）**：工具执行层（agenttools.ExecutionContext）对 data_classification 全盲（一等工具亦然，MCP 不是第一个出口）；MCP 默认关闭（无 manifest = 零出口），首次配置清单的 PR 必须触发前置依赖——把 Classification 引入 ExecutionContext + production 分类默认拒绝外部调用——作为启用票一部分，闸门绑在真实启用动作上。④ 拒绝码：边界拦截 = business_failure + 稳定 reason 前缀（OutboundBlockedReason），与外部工具自身失败（business_failure/timeout）可区分。⑤ MCP 工具一律未认证，索取受保护度量落 Reject（有测试）。登记：MCP 工具绕过 collector 计数（其失败模式是构造期拒绝非静默跳过）；副本可用性随启动时刻变化（诚实缺席，部署纪律）；审批模型等价（同一 registry → 九控制自动生效） | L3 工单 L3-D；`tmp/delivery-L3-D.md` |
| **D41** | **治理链拒绝的错误码保真（RC1，2026-08-24）。** guard 路径此前把九个控制的**所有**拒绝一律压成 `scope_denied`，并把治理链内部 sentinel 文案绕过 `publicPolicyError` 原样外抛。裁决：**`scope_denied` 是底线 1（跨法人越界）的专属证据码**，不得被能力不足/权限不足/缺幂等键/预算耗尽等其他拒绝伪装——审计里读到它必须能断定是租户越界，否则这个码作为证据就失效了。实现形态：**`GuardResult` 携带错误码，由各控制在 deny 时明确带上**，不在 runtime 侧靠字符串匹配 `Reason` 反推 sentinel（文案微调即静默失配，比现状更糟）。硬断言：装 guard 与不装 guard 两条路径，对同一拒绝场景返回**同一个码**——两条路径分叉当初没被发现，正是因为没有这条断言 | 既有纪律「权限拒绝必须保持 `scope_denied` 不得软化」只覆盖了**软化**方向，未覆盖**伪装**方向 | `7107af6`；`agenttools/runtime.go` 的 `policyErrorCode`/`publicPolicyError` |
| **D42** | **会话闸门保护不了不加载会话的 handler（SI1 + SI2，2026-08-25）。** SI1 关掉了会话续接的跨法人缺口后，SI2 复验发现同一份内容从直读路径照给：`GetRunByID` / `GetMessageByID` / `GetArtifactByID` / `GetReviewActionByID` / `GetRunCheckpoint` 只按 `user_id` 过滤，`ListRunEvents` 无任何归属过滤，而调用它们的 handler 不加载会话、因而不经过 SI1 建立的任何闸门。裁决：**法人边界必须落在仓储方法签名上，不能靠上游闸门代劳**——「经会话闸门保护的直连端点」这个说法实测不成立。推论纪律：新增任何 run/artifact/message 直读端点时，边界参数是签名的一部分，不是调用方的自觉 | SI1 曾把直读面登记为「经会话闸门保护」，SI2 实测推翻 | `acee42c` / `4bd451a`（SI1）、`6e95ece`（SI2） |
| **D43** | **实测与估算不得合并成一个数（AF1-a → RT1-A，2026-08-24/25）。** `ai_chat_messages.measured_tokens` 存的是 `prompt_tokens`（**整轮 prompt 总量**），而装配器把每行该值**逐条相加**，误差随轮数二次增长（探针：3 轮实测 1000/1100/1200 → counted=3303）。改为「取最近一轮实测为基线 + 只对基线之后的尾部估算」。**登记为纪律而非一次修复**：(a) 指标层必须把实测与估算**分成两条 series** 暴露，把两者加总成单一数字比没有仪表盘更糟——它会被当成测量结果拿去做决策；(b) **测试输入的语义必须与生产写入的语义一致**——这个缺陷单测照不出来，正是因为测试喂的是「逐条」语义而生产写的是「轮总量」语义 | 「类型对了就是对了」——同为 int64 的 token 数，语义不同即算错 | `5569048`；`contextassembler/impl.go`；`lease_agent_context_tokens_{measured,estimated}_total` |
| **D44** | **C1 第三层六项裁剪为三项（2026-08-25 裁决，2026-08-26 收口）。** **做**：MCP 接入（唯一能减少后续工作量的——接外部工具不必每次改 `aiagent`）、定时触发（`RecoverRunLeases` 两侧都实现了却全仓零调用方，靠它才能关这个缺口）、健康检查（原 `/health` 探测失败仍返回 200 + `status:ok`，修它不是新增能力而是修正既有谎报）。**推迟三项及重开条件**：**AR4 子任务委派**——新增能力同时新增攻击面（D-C8），且是六项里唯一需要 picoclaw 完整回合循环做地基的（未决 #12），重开条件是出现真实负载（如「逐店归因」批量任务）+ 未决 #12 裁决；**模型路由**——成本/质量优化，在没有生产流量数据前无判据，重开条件是 AR3 观察窗口（`CONTEXT_ASSEMBLER_MODE=count`）出数；**AR6 跨会话记忆**——依赖 AR3 真正在生产上跑过，且它是新的跨租户泄漏面，重开条件同上加一条隔离设计评审。**推迟不是取消**：`ContextKey` 五维隔离键与 `narrow` 求交的设计留在模块深化文档里，未来重开接口不用重画 | C1 母 spec 原列六项能力并列推进 | L3 工单「范围裁决」；模块深化 AR4/AR6 |
...
| **G11 completed run 持有不释放的过期租约（新登记，RT1-L3-C 复验，2026-08-26）** | 🟡 **事实登记，本票不改行为**：`UpdateClaimedRunStatus` 只写 status 不清租约（ReleaseRunLease 才清），因此 completed 的 run 会带着永不释放的过期 leased_until 长期存在。今天靠 lease recovery 的 status='running' 谓词兜住「已完成工作被重跑」（见 TestLeaseRecoveryJobDoesNotResurrectCompletedRuns）——这是一条隐含依赖，不是设计意图。要不要在状态终态时清租约，另开票裁决；在那之前任何改动 recovery 谓词或 UpdateClaimedRunStatus 的人必须读这条 | agent_run_queue.go:300；tmp/delivery-L3-C.md |
...
>
> **RT1-D-2 旧链删除（2026-08-26）**：`internal/agentcore/hooks` 整包删除（W2 六前+三后实现）。事实基础：生产装配中它只剩一个挂点（`aiagent` 的 base runtime），而 chat/gateway 均以 WithGuard 覆盖、唯一不被覆盖的 `Agent.Execute` 无生产调用方——旧链已是死代码。
>
> **C1 第三层已交付三项（2026-08-26，L3-A~D，范围裁决见 D44）**：L3-B 健康检查（`/health` 拆成 liveness/readiness，依赖探测失败不再返回 200+`status:ok`，内部错误原文不外泄）；L3-C 定时触发框架（`internal/scheduler` 仅依赖标准库 + lease recovery 首次有调用方，两类分治见 D39）；L3-D MCP 接入（第一方最小客户端，白名单出站边界 + env 白名单，见 D40）。**L3-D 经两轮复验返工**（`8607fa2`）：客户端层七个缺陷——超时毒化、并发数据竞争、握手无超时、无界行缓冲、写阻塞无视 deadline、白名单对省略 `type` 全绕过、server 主动请求吃掉 pending id——各配回归测试并逐个变异实证。**未完**：L3-A 的 A-3 观察窗口（`CONTEXT_ASSEMBLER_MODE=count`，≥7 天，口径见 `tmp/L3-A-observation-plan.md`）尚在采集，其数据是模型路由重开的前置判据。**不得据此写成「C1 完成」**——推迟三项见 D44。语义选择：**base 回落 Evaluate**（不换新链）——逐行比对证实其判定与 Evaluate 同构、码走同一映射器；三边比对（新链/旧链/Evaluate，9 场景含 TenantScope 不对称行）证实删除零漂移。等价性的适用范围有明确边界：ProtectedMeasure / BudgetGuard 不在 Evaluate 内（两条路径的生产 Deps 里本就是 nil），TenantScope 身份完整性链侧严于 Evaluate（AR5d 决策）——锚测试 `governance/dualpath_rc1_test.go` 注释与 `TestDualPathKnownAsymmetryTenantScopeCompleteness` 锁定。`AgentToolRuntime()`（零生产调用方，仅靠一条测试存活）一并删除，反向对照改为直接构造 base runtime。`internal/agentcore` 本体保留（llm/stream.go 在用）。
...
| 14 | **RT1-OPEN-1：部署级机器信任的两个同场裁决项。** (a) **worker→法人绑定**——worker 池从共享队列取队、不按法人分区（部署级信任的选择，不是结构保证；RT1-B 已如实登记并订正）。若需要分租户 worker（worker 只服务特定法人），需新授权策略（worker→法人绑定表）。(b) **Type B 定时任务的 Principal 来源**（2026-08-26 并入，见 D39）——业务运行型定时任务产生真实 run、必须走治理链并从登记表取 Principal（scope 不宽于登记者），但**登记表本身放表还是放配置未定**，与 (a) 是同一个信任模型问题的两面，同场裁决。Type A 系统维护型已按 D39 豁免，不受本项阻塞。**独立开放决策，不随 G9 关闭。** | 分租户部署；Story 29 等 Type B 定时任务 | D39；`5cc1953` |
