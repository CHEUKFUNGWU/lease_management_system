...
每批独立可交付、独立验收；优先级排序：可信度 → 日常价值 → 差异化。**推荐波次（2026-08-16 评审，消除「顺序即优先级」与「编号不代表先后」的并置歧义）**：Wave 1 = P0 + P5-1；Wave 2 = P1 + P5-2；Wave 3 = P2 + P5-3（联动）；Wave 4 = P3；Wave 5 = P4。**两条全局纪律（2026-08-16 评审）**：凡动 schema（M6 DB 适配器、M7 快照、M8 导入留痕、P2 版本状态）一律增量迁移 + `01_init.sql` 双交付；P3-32 / P3-33 / P3-36 / P5-48 的「二选一」在看板登记任务前定死并标注决策人，批次验收不留移动靶。所有批次遵守五条底线与「模拟/正式」标识纪律，导出与决策卡输出必须带 Working/模拟标注。
