...
> 所有现状描述均已在仓库中验证（2026-08-15 两份 Review——BP/AI 双视角与数据链路「文件 → AI 解析 → 入库 → BI 展示」——实锤 + 代码复核；数据链路 9 项断言全部复核通过）。2026-08-16 walkthrough 评审再次复核 24 项断言（22 项属实、2 项修正已并入正文），并补齐 4 项接口级缺口：M3 接缝按栈拆分、M2 行为变更决策、M8 信封补全与门店身份解析、schema 变更双交付纪律。同日 main 78 commit（UIUX 四阶段、home 改版、Sankey、取数接缝）合并后全量断言复核：P0–P5 诊断全部仍成立；对接面刷新（命名、前端底座约束、GUARD-001 证据规则、会话语义与路由加词现状）已并入正文。
...
| AI 路由 | 关键词匹配 skill registry（2026-08-16 合并后 main 已增量加入「门店经营利润」「四墙利润」两词） | 匹配策略仍浅：「A 门店毛利下滑」这种自然问法漏匹配 |
...
**关键行为变更**：基准约定统一为 `(c−p)/|p|`（分母取绝对值）作为唯一契约，pulse 的 `(c/p−1)` 弃用。从 −100 改善到 −50 必须显示为「改善」，两个页面一致。这是行为变更，需回归既有展示与测试；且属 AGENTS.md GUARD-001「替换类改动」——验收必须证明新公式在展示点生效（运行时对照或规则体断言），只证旧公式消失不算（2026-08-16 合并后新增规则）。
...
**藏在接口后面**：日历月/季的起止边界与上一等长比较期的解析；`as_of` 与期间类型的交互（as_of 是日历日锚点，滚动窗口从它往回推，日历期以它为「落在哪个月/季」）；「上月」「本季度」的自然语言 → 期间映射（P2 前先支持显式 `YYYY-MM` / `YYYY-Qn`）；与既有 7/14/28 校验的兼容收口。**前端先例（2026-08-16 合并后）**：/performance 已落地 periodDraft → period 草稿/应用分离 + `YYYY-MM` 格式守卫（FIX-026，带测试）——期间应用的交互范式沿用此形状；期间选项清单落地时按 CONTRACT-001 形状登记进 code-lists 契约测试。
...
**藏在接口后面**：`GroupBy=""` 时行为与现在完全一致（零回归）；有 GroupBy 时走语义层聚合，并把信号规则应用到分组级（区域级关注榜）；分组内多币种处理（分区或显式降级）；分组样本门槛沿用 MinimumPeerCount 语义；分组的 drilldown URL 模板。前端加分组切换，复用既有响应契约——分组切换经既有 `useRetailQuery` 取数接缝（零售三页已被机械测试强制走它，不得自建竞态门），group_by 选项清单按 CONTRACT-001 形状登记进 code-lists 契约测试（2026-08-16 合并后底座约束）。
...
**接口不变**：`Select(intent Intent) (Definition, bool)`。**实现加深**：高频经营词并入 retail skill 匹配（毛利、下滑、为什么、同群、经营利润、四墙、闭店、异常…）+ 中文表达变体（“为咩”“为何”）+ 数值特征（门店名 + 指标词）+ 兜底顺序与优先级。2026-08-16 合并后 main 已增量加入「门店经营利润」「四墙利润」两词（FIX-029，只增不减）——方向一致，自然问法漏匹配仍在，本项继续推进。深度判定：匹配策略的复杂度从调用方移入 registry，调用方接口不膨胀。**明确不引入** LLM function calling 主链路（保持确定性，与治理模型一致）。
...
- **会话持久化**：零售页内嵌 AI 抽屉的对话上下文跨页面往返保留，或明确定义一次性会话语义并在 UI 提示（PRD P3-32 二选一）。**2026-08-16 合并后证据**：服务端会话已是一等持久实体（042/043 迁移引入 `initiator ∈ {user, system}`，system 会话完整留痕、仅从用户列表投影过滤）——「跨页面保留」的实现路径已大半铺好，倾向此选项，「一次性会话」弱化为备选。测试：页面往返后上下文保留的组件测试。
- **死代码清理**：既有深链构造被真实接线调用，或确认无调用方后删除（PRD P3-33 二选一）。**2026-08-16 合并后证据**：main 侧反向测试已钉死方向（ai-002 断言三页不得 push 深链、页内走 Drawer）——二选一事实定为**删除**，删除时同步移除该反向断言与死代码。测试：移除后全仓无引用。
...
**接缝与适配器**：新 handler 端点（沿用既有 store-day 写入权限）+ 新「经营数据导入」页。适配器：受控模板解析（Go 确定性）与 AI 列映射建议（LLM adapter，Assist Mode）两个适配器 ⇒ 真实接缝；AI 输出只到映射建议为止，人工确认后才进入确定性路径。**前端底座（2026-08-16 合并后）**：导入页取数经 `useRetailQuery`，校验报告的 loading/error/empty 呈现走 `classifyDataState`/StateBlock，表格走 tableScrollX 容器原语（DESIGN.md §8.1）；「标准字段清单」若在前端渲染选项，属 CONTRACT-001 的双清单形状，登记进 code-lists 契约测试。
...
- M3：口径头、转义、文件名、分类标识；公式（结构化引用 + 缓存值一致）、除零保护、`= + - @` 注入转义；表对象（totalsRow/筛选/冻结）；表名/列名转义（表头含 `[ ] # '`）；PPTX 原生对象断言；描述符两栈一致性；handler 集成（`format` 参数）与超限降级。
...
- M8：幂等重放、行级错误与部分成功、覆盖率报告、envelope 强制、fact_version / dataset_version 赋值与修正重传、门店身份解析（未匹配/跨法人）、分批边界；AI 映射建议不含数值；集成断言只写 store-day、零写 IFRS 16 正式表。
...
每批独立可交付、独立验收；优先级排序：可信度 → 日常价值 → 差异化。**推荐波次（2026-08-16 评审，消除「顺序即优先级」与「编号不代表先后」的并置歧义）**：Wave 1 = P0 + P5-1；Wave 2 = P1 + P5-2；Wave 3 = P2 + P5-3（联动）；Wave 4 = P3；Wave 5 = P4。**三条全局纪律（2026-08-16 评审 + 合并后补充）**：凡动 schema（M6 DB 适配器、M7 快照、M8 导入留痕、P2 版本状态）一律增量迁移 + `01_init.sql` 双交付；P3-32 / P3-36 / P5-48 的「二选一」在看板登记任务前定死并标注决策人（P3-33 已由合并后证据定向「删除」），批次验收不留移动靶；涉及前端的批次（M2 页面、M5、M8 导入页、P1 各页）一律经 `useRetailQuery` 与 `classifyDataState`/StateBlock 契约，新枚举选项登记 code-lists 契约测试，容器遵守 DESIGN.md §8.1。所有批次遵守五条底线与「模拟/正式」标识纪律，导出与决策卡输出必须带 Working/模拟标注。
