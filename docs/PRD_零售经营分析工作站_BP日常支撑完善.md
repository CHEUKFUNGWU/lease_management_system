...
- **导出模块**：行结构、口径头、CSV 转义、文件名与分类标识；**公式用例**（合计/差异/除零保护、公式字符串含结构化引用且缓存值与受控响应一致的回读断言、`= + - @` 注入转义）；**表对象用例**（命名表存在、totalsRow 合计、筛选按钮、冻结表头）；PPTX 解包断言原生对象存在（非图片）；handler 集成测试带 `format` 参数。先例：既有 FP&A 行动导出 CSV 测试。
