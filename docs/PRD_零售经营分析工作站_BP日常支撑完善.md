...
3. **平台导出模块（新）。** 导出行为与口径头只实现一次，**平台级**能力：零售三页先行，其余功能页按同一列/公式描述符接入。输入为各服务的响应类型 + envelope（classification / dataset_version / as_of / formula_version / period），输出 CSV（服务端，口径权威、可审计复演）、**带公式的 XLSX**（服务端 excelize，MIT、纯 Go；公式 + 打开时自动重算，不依赖计算引擎）与**可编辑 PPTX**（前端 pptxgenjs，MIT、浏览器端；原生文本框/表格/图表对象）。每个导出 kind 声明自己的列/公式描述符（合计 SUM、列间算术、比率 IF 除零保护），模块统一生成公式并防护公式注入（数据文本中 `= + - @` 开头一律转义）。导出与展示共享同一响应，不重读仓库（对齐既有 projection 纪律）。文件命名与表头带 Working/模拟标识。支撑 P1-13/14/19。
...
- **导出模块**：行结构、口径头、CSV 转义、文件名与分类标识；**公式用例**（合计/差异/除零保护、公式字符串与缓存值一致性回读断言、`= + - @` 注入转义）；PPTX 解包断言原生对象存在（非图片）；handler 集成测试带 `format` 参数。先例：既有 FP&A 行动导出 CSV 测试。
...
- **导出增强（2026-08 用户确认）**：带公式 XLSX 由服务端 excelize（MIT、纯 Go）生成（公式 + 打开时自动重算，不依赖计算引擎；实施时验证所选库能否同时写缓存值）；可编辑 PPTX 由前端 pptxgenjs（MIT）生成原生对象（文本框/表格/图表）；口径头与 Working/模拟标识三格式一致；既有报表页的 SheetJS 导出保持不动（底线：不破坏既有功能）。classification 过滤（P1-18）属「值得收紧」项，聚合路径已安全，本轮只收紧列表读取面。
