...
> 相关：[母 spec C1](agent-runtime-overhaul-c1.md) · [模块深化 AR1–AR6](../CodebaseDesign_AgentRuntime升级_模块深化.md) · [ADR-0028 + Correction](../adr/0028-extend-picoclaw-adoption-to-the-whole-runtime.md) · [AI 文档索引 §5 未决 #9 / #12 / #14](../AI_文档索引与现行决策.md) · [RT1](agent-runtime-c1-remaining-work.md)
...
- README 状态行：C1 从「前两层完成，第三层未开始」改为如实描述——三项已做、三项按 2026-08-25 裁决推迟，**不要写成「C1 完成」**
- 文档索引 §5：第 9 项按 A-2 的结论关闭或转为带日期的延期；第 12 项按 A-1 重写阻塞范围；第 14 项按 L3-C 的 Principal 方案更新
- 母 spec C1 的能力清单：健康检查那一格的「零（全仓无 `/health` 类端点）」按 L3-B 订正
- 推迟的三项（AR4 / 模型路由 / AR6）在索引里留一条可检索的登记，含推迟日期与重开条件
