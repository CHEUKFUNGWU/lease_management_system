...
**第三，runtime 缺少一批本该有的基础能力。** MCP（全仓零）、模型路由、定时触发、健康检查、跨会话记忆——这些不是锦上添花，是「agent runtime」这个词的通常含义的一部分。缺了它们，每一个新需求都要在 `aiagent` 那 6603 行里再接一根线，那个文件已经是全仓最大的单包。
...
**第三层：能力补齐。** 子任务委派、MCP、模型路由、定时、健康检查、记忆，按价值排序逐个接入，每个都是可独立回退的增量。
...
30. 作为运维，我想有健康检查反映 runtime 与其依赖的实际状态，以便故障可被发现而不是被用户报告
...
| **子任务委派** | **零** | ~~取自 picoclaw~~（D-C8）→ **推迟，D44**（新增攻击面；且是六项里唯一需要 picoclaw 完整回合循环做地基的，未决 #12）。接口设计保留 |
| **MCP** | **零** | ~~取自 picoclaw~~（D-C5）→ **已交付（L3-D，`7b2f1c2`+`8607fa2`）。口径订正：不是取自 picoclaw，是第一方最小实现**——手写 JSON-RPC 子集（initialize / tools/list / tools/call）+ stdio 传输，不引第三方 SDK，理由是保持审计面最小。边界与返工结论见 D40 |
| **模型路由** | **零** | ~~取自 picoclaw~~ → **推迟，D44**（成本/质量优化，在 AR3 观察窗口出数前无判据） |
| **定时触发** | **零** | ~~取自 picoclaw~~ → **已交付（L3-C，`5cc1953`）。口径订正：第一方实现**，`internal/scheduler` 仅依赖标准库并有双向 import guard。两类分治见 D39 |
| **健康检查** | ~~**零**（全仓无 `/health` 类端点）~~ → **订正（L3-B）：这句是错的。** `cmd/api/main.go` 一直有 `/health`，但它**比没有更糟**——数据库探测失败仍返回 200 且 `status:"ok"`，任何按状态码或按该字段判活的探针都会认为服务健康（风险红线 12「假装在检查」的运行期形态）；且把内部错误原文吐给未认证调用方 | **已交付（L3-B，`1a73bc1`）**：liveness / readiness 分离，依赖真实状态进判定，内部错误不外泄 |
| 心跳 | 已有（worker 租约心跳 `HeartbeatRunLease`，runner 定期续租） | 留 |
| **跨会话记忆** | **零** | ~~取自 picoclaw~~ → **推迟，D44**（依赖 AR3 真正在生产上跑过，且是新的跨租户泄漏面）。`ContextKey` 五维隔离键与 `narrow` 求交设计保留 |
...
> **处置结果（2026-08-26）**：六项「零」中三项已交付（MCP / 定时触发 / 健康检查），三项**显式推迟并带重开条件**（子任务委派 / 模型路由 / 跨会话记忆，见 D44）。推迟是处置的一种，不是遗漏——但**「都必须有」这句在今天尚未兑现**，不得据此把 C1 写成完成。另有两处口径订正随交付浮现：MCP 与定时触发最终是**第一方实现**而非取自 picoclaw；健康检查那格的「零」是错的（有端点，但它是「假装在检查」）。
...
1. **计数**：token 计数与所用模型对齐。**2026-08-24 修订（学 pi，登记 D37）**：采用双轨——provider 实测 usage（每轮响应回传）为主真值，仅未发送的尾部消息用保守估算兜底、下一轮即被真值覆盖；不建本地分词器注册制。原「缺失即拒绝」语义保留给预算几何（窗口大小是配置）。估算的误差会在预算边界处系统性失效，而那正是唯一重要的地方。
