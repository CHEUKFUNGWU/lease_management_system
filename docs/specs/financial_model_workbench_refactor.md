...
## 追加决策（2026-08-22，用户复核后）：让会计/FP&A/Finance BP 读得懂

用户判定：重构后左栏仍是工程师语言，BP 看不懂。按责任分三层，①②为前端追加批次：

**① 文案层（前端，立即可做）**
- PageHeader 副标题人话化：说明页面做什么（把经营假设推演成三张报表，先对账、再出数、后发布）
- 卡片加步骤化副标（①选模型 ②填假设 ③校期初 ④运行 ⑤发布）
- 假设键提示映射（i18n ASSUMPTION_HINTS）：已知键给中文名+含义（sssg=同店销售增长率、gross_margin_rate=毛利率），未知键原样显示
- 三道闸逐条 tooltip：自平衡=资产=负债+权益；归并一致；租赁余额与引擎对照
- 「填充示例」按钮：一键填入合法示例，让用户看见合法输入长什么样
- 示例假设键从种子测试键（sssg）换成语义化示例

**② 交互层（前端）**
- 期初闸目标流程改为「选期间 → 引擎侧自动取数 → 导入侧上传 → 校验」；手打合约行降级为高级模式

**③ 地基层（后端立票，B 阶段扩展）**
- 定义目录实装 + 业务名（"FY2026 年度预算模型"），非 UUID
- 假设键字典端点（label/unit/description 按模板暴露）
- 期初导入端点（引擎侧自动取自计量结果/上期 run；导入侧文件上传）

验收口径：一个不懂实现的 FP&A 能只读页面文案，说出每张卡片是什么、下一步该做什么。
