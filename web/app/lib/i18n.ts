...
const dict: TranslationDict = {
  ...
  "storepnl.formula_hint": { "zh-CN": "公式（白名单 DSL，如 rows.revenue - rows.labor_cost；后端保存时校验）", "zh-HK": "公式（白名單 DSL，如 rows.revenue - rows.labor_cost；後端保存時校驗）", en: "Formula (whitelist DSL, e.g. rows.revenue - rows.labor_cost; validated on save)" },
  ...
  "finmodel.basis_note": { "zh-CN": "管理口径三表模型；勾稽不绿的 run 不可发布", "zh-HK": "管理口徑三表模型；勾稽不綠的 run 不可發佈", en: "Management-basis three-statement model; tie-outs must pass before publishing" },
  "finmodel.assumptions_hint": { "zh-CN": "用 JSON 键值对填写业务假设；下方会把每个已知键翻译成人话。所有假设都会版本化留痕。", "zh-HK": "用 JSON 鍵值對填寫業務假設；下方會把每個已知鍵翻譯成人話。所有假設都會版本化留痕。", en: "Fill business assumptions as a JSON object; known keys are decoded below. Every assumption is versioned." },
  ...
  "finmodel.gaps": { "zh-CN": "缺口清单（具名 Gap）", "zh-HK": "缺口清單（具名 Gap）", en: "Named gaps" },
  "finmodel.tie_out_gate_note": { "zh-CN": "勾稽未全绿：此 run 不能发布为计划版本", "zh-HK": "勾稽未全綠：此 run 不能發佈為計劃版本", en: "Tie-outs are not all green: this run cannot be published as a plan version" },
  ...
  "finmodel.opening_auto_unavailable": { "zh-CN": "端点尚未开放（B 阶段）：上线后将自动读取计量结果与上期 run 的期初余额，无需手工填写合约行", "zh-HK": "端點尚未開放（B 階段）：上線後將自動讀取計量結果與上期 run 的期初餘額，無需手工填寫合約行", en: "Endpoint not live yet (stage B): once live, balances come straight from measurement results and the prior run — no manual rows needed" },
  "finmodel.opening_step3": { "zh-CN": "第 3 步 · 导入侧上传", "zh-HK": "第 3 步 · 導入側上傳", en: "Step 3 · Import-side upload" },
  "finmodel.opening_import_unavailable": { "zh-CN": "上传端点尚未开放（B 阶段）：上线后可直接上传导出的期初余额文件", "zh-HK": "上傳端點尚未開放（B 階段）：上線後可直接上傳導出的期初餘額文件", en: "Upload endpoint not live yet (stage B): once live, you can upload the exported opening-balance file directly" },
