...
function MetricTable({ response, selectedKey, notReady }: { response: RetailScenarioResponse; selectedKey: string; notReady?: boolean }) {
  ...
  return <Table size="small" pagination={false} rowKey="code" dataSource={rows} columns={[
    ...
    { title: "状态", render: (_: unknown, row: typeof rows[number]) => <Flex align="center" gap={4}>{notReady && <KPIReadyBadge />}<StatusTag kind={row.result?.status === "complete" ? "success" : "warning"}>{row.result?.status || "unavailable"}{row.result?.reason ? ` · ${row.result.reason}` : ""}</StatusTag></Flex> },
  ]} />;
}
