...
function ScenarioPageInner() {
  ...
  const optionsError = optionsState.kind === "failed" ? optionsState.message ?? null : null;
  ...
  return <ProtectedRoute><AppLayout><div className="scenario-workbench-page">
    ...
    {!query.storeID && <Card className="scenario-block-gap"><Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={t("scenario.pick_store", language)} /></Card>}
    ...
    {optionsError && <Alert className="scenario-block-gap" type="error" showIcon message={t("scenario.options_error", language)} description={optionsError} action={<Button size="small" onClick={optionsRetry}>{t("common.retry", language)}</Button>} />}
    {query.classification && !loadingOptions && !optionsError && options.length === 0 && <Alert className="scenario-block-gap" type="info" showIcon message={t("scenario.no_authorized", language)} description={t("scenario.no_authorized_desc", language)} />}
