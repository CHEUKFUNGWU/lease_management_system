...
function ScenarioPageInner() {
  ...
  return <ProtectedRoute><AppLayout><div className="scenario-workbench-page">
    <PageHeader title={t("scenario.title", language)} subtitle={t("scenario.subtitle", language)} primaryAction={<Button type="primary" icon={<PlayCircleOutlined />} loading={loading} onClick={evaluate}>{t("scenario.calculate", language)}</Button>} secondaryAction={<Space><Button onClick={() => router.push(retailAIHref({ page: "scenario-workbench", title: t("scenario.title", language), asOf: query.asOf, windowDays: query.windowDays, classification: query.classification, datasetVersion: query.datasetVersion || undefined, sourceSystem: query.sourceSystem, storeID: query.storeID || undefined, horizonMonths: horizon, assumptions }))}>{t("common.ai_analysis", language)}</Button><Button icon={<ArrowLeftOutlined />} onClick={() => router.push(backURL)}>{t("scenario.back_store360", language)}</Button></Space>} />
