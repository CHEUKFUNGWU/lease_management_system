...
function ScenarioPageInner() {
  ...
  // FETCH-001: storeOptions runs through the shared fetch seam (race gate / token injection).
  ...
  const { loading: optionsLoading, state: optionsState, retry: optionsRetry } = useRetailQuery({
    token,
    params: optionParams,
    paramsKey: `options:${query.classification}|${query.datasetVersion}|${optionsRetryNonce}`,
  ...
  const setQuery = (next: Partial<typeof query>) => {
    ...
    // P0-5: write the seven levers + horizon back so shared links reproduce
    // the exact assumptions — symmetric with parseURL, which reads them.
  ...
  return <ProtectedRoute><AppLayout><div className="scenario-workbench-page">
    <PageHeader title={t("scenario.title", language)} meta={t("scenario.scope_note", language)} help={<HelpTrigger content={scenarioHelpContent(language)} language={language} />} primaryAction={<Button type="primary" icon={<PlayCircleOutlined />} loading={loading} onClick={evaluate}>{t("scenario.calculate", language)}</Button>} secondaryAction={<Space><RetailExportMenu kind="scenario" disabled={!response} envelope={response ? envelopeFromScenario(response) : null} rows={() => (response ? scenarioRowsFromResponse(response, selectedKey) : [])} /><Button onClick={() => setAiOpen(true)}>{t("common.ai_analysis", language)}</Button><Button icon={<ArrowLeftOutlined />} onClick={() => router.push(backURL)}>{t("scenario.back_store360", language)}</Button></Space>} />
    ...
    {query.classification && !loadingOptions && optionsState.kind !== "failed" && options.length === 0 && <Alert className="scenario-block-gap" type="info" showIcon message={t("scenario.no_authorized", language)} description={t("scenario.no_authorized_desc", language)} />}
    {response && !freshResponse && <Alert className="scenario-block-gap" type="warning" showIcon message={t("scenario.stale_title", language)} description={t("scenario.stale_desc", language)} action={<Button size="small" onClick={evaluate}>{t("scenario.recalculate", language)}</Button>} />}
    {selectedStore && <Card title={<Flex justify="space-between"><span>{selectedStore.store_code} · {selectedStore.store_name}</span><Typography.Text type="secondary">{selectedStore.brand} · {selectedStore.region}</Typography.Text></Flex>} className="scenario-block-gap"><Row gutter={[12, 12]}>{DELTA_FIELDS.map((field) => <Col xs={24} sm={12} lg={8} key={field.key}><Typography.Text>{t(field.labelKey, language)}（{field.unit === "pp" ? "pp" : "%"}）</Typography.Text><InputNumber className="scenario-full-width" value={assumptions[field.key]} onChange={(value) => { const next = { ...assumptions, [field.key]: value ?? 0 }; setAssumptions(next); setQuery({ assumptions: next }); }} min={field.unit === "pp" ? -100 : -100} max={field.unit === "pp" ? 100 : 300} /></Col>)}</Row><Flex gap={12} wrap="wrap" align="center" className="scenario-block-gap"><span>{t("scenario.horizon", language)}</span><Select value={horizon} options={[3, 6, 12].map((value) => ({ value, label: t("scenario.months_option", language).replace("{n}", String(value)) }))} onChange={(value) => { setHorizon(value); setQuery({ horizon: value }); }} /><Typography.Text type="secondary">{t("scenario.baseline_note", language)}</Typography.Text></Flex></Card>}
    {error && <Alert className="scenario-block-gap" type="error" showIcon message={t("scenario.unavailable", language)} description={error} />}
    {loading && <Card className="scenario-block-gap"><Flex justify="center" className="scenario-loading-block"><Spin tip={t("scenario.loading", language)} /></Flex></Card>}
    {response && <><Card className="scenario-block-gap" title={<Flex justify="space-between"><span>{t("scenario.baseline_title", language).replace("{name}", response.scenarios.find((item) => item.key === selectedKey)?.name || "Plan")}</span><StatusTag kind={freshResponse ? "success" : "warning"}>{freshResponse ? "review_required · Working" : t("scenario.stale", language)}</StatusTag></Flex>}><div className="scenario-block-gap-sm"><Radio.Group optionType="button" buttonStyle="solid" value={selectedKey} onChange={(event) => setSelectedKey(event.target.value)} options={response.scenarios.map((item) => ({ label: item.key === "baseline" ? t("scenario.option_baseline", language) : item.name, value: item.key }))} /></div><MetricTable language={language} response={response} selectedKey={selectedKey} notReady={!response.envelope?.decision_ready} /><Flex gap={8} wrap="wrap" className="scenario-block-gap-sm"><Tag>{t("scenario.monthly_change", language)}：{formatScenarioValue(response.scenarios.find((item) => item.key === selectedKey)?.monthly_contribution_change, "currency", response.currency, language)}</Tag><Tag>{responseHorizonLabel(response, language)}：{formatScenarioValue(response.scenarios.find((item) => item.key === selectedKey)?.horizon_contribution_change, "currency", response.currency, language)}</Tag></Flex></Card><Collapse className="scenario-block-gap" items={[{ key: "evidence", label: t("scenario.evidence_title", language), children: <Space direction="vertical"><Typography.Text>{t("scenario.evidence.facts", language).replace("{from}", response.evidence.current.date_from).replace("{to}", response.evidence.current.date_to).replace("{observed}", String(response.evidence.observed_store_days)).replace("{expected}", String(response.evidence.expected_store_days)).replace("{rate}", response.evidence.coverage_rate == null ? "—" : `${response.evidence.coverage_rate.toFixed(2)}%`)}</Typography.Text><Typography.Text>classification={response.data_classification} · dataset={response.dataset_version || "—"} · source={response.source_system || response.evidence.source_systems.join(", ") || "—"}</Typography.Text><Typography.Text>fact version={response.evidence.fact_version_min}–{response.evidence.fact_version_max} · formula={response.formula_version} · scenario={response.scenario_version}</Typography.Text><Typography.Text>required fields：{response.evidence.required_fields.join(", ")}</Typography.Text><Typography.Text>{t("scenario.evidence.formula", language)}</Typography.Text><a href={response.evidence.kpi_drilldown_url}>{t("scenario.view_drilldown", language)}</a></Space> }]} /><Row gutter={[16, 16]} className="scenario-block-gap"><Col xs={24} lg={14}><Card title={t("scenario.bridge.title", language)}><Table size="small" pagination={false} rowKey="code" dataSource={response.scenarios.find((item) => item.key === selectedKey)?.bridge.items || []} columns={[{ title: t("scenario.bridge.item", language), dataIndex: "label" }, { title: t("scenario.bridge.contribution", language), render: (_: unknown, item: { contribution: number | null; unit: string }) => formatScenarioValue(item.contribution, item.unit, response.currency, language) }]} /><Typography.Text type="secondary">{t("scenario.bridge.total", language)} {formatScenarioValue(response.scenarios.find((item) => item.key === selectedKey)?.bridge.total_change, "currency", response.currency, language)} · {t("scenario.bridge.residual", language)} {formatScenarioValue(response.scenarios.find((item) => item.key === selectedKey)?.bridge.rounding_residual, "currency", response.currency, language)} · {t("scenario.bridge.conservation", language)} {bridgeConservation(response.scenarios.find((item) => item.key === selectedKey)?.bridge || { items: [], total_change: null, rounding_residual: null, status: "unavailable" })}</Typography.Text></Card></Col><Col xs={24} lg={10}><Card title={t("scenario.draft.title", language)}><Space direction="vertical" className="scenario-full-width"><Input value={title} onChange={(event) => setTitle(event.target.value)} placeholder={t("scenario.draft.title_placeholder", language)} /><Input.TextArea value={plannedAction} onChange={(event) => setPlannedAction(event.target.value)} placeholder={t("scenario.draft.action_placeholder", language)} rows={3} /><Flex gap={8}><Input value={ownerName} onChange={(event) => setOwnerName(event.target.value)} placeholder={t("scenario.draft.owner_placeholder", language)} /><DatePicker allowClear value={dueDate ? dayjs(dueDate) : null} onChange={(value) => setDueDate(value ? value.format("YYYY-MM-DD") : null)} /></Flex><Input value={verificationPeriod} onChange={(event) => setVerificationPeriod(event.target.value)} placeholder={t("scenario.draft.period_placeholder", language)} /><Button icon={<SaveOutlined />} type="primary" loading={saving} disabled={selectedKey === "baseline" || !canSaveScenario(response, selectedKey, responseKey, currentEvaluationKey) || !title.trim() || !plannedAction.trim()} onClick={saveAction}>{t("scenario.draft.save", language)}</Button>{actionResult && <Alert type="success" message={t("scenario.draft.saved_title", language)} description={<Space direction="vertical"><span>{t("scenario.draft.real_id", language).replace("{id}", String(actionResult.data.id || "—")).replace("{status}", String(actionResult.data.status || "open"))}</span><span>Owner：{String(actionResult.data.owner_name || "—")} · Due：{String(actionResult.data.due_date || "—")}</span><span>{t("scenario.draft.idempotency", language).replace("{value}", actionResult.idempotentReplay ? t("scenario.draft.idempotent_yes", language) : t("scenario.draft.idempotent_no", language))}</span><a href="/performance">{t("scenario.draft.go_workbench", language)}</a></Space>} />}</Space></Card></Col></Row></>}
...
export default function ScenarioWorkbenchPage() {
  return <Suspense fallback={<div className="scenario-suspense-fallback"><Spin /></div>}><ScenarioPageInner /></Suspense>;
}
