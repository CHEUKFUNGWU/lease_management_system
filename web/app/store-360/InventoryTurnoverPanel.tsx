...
export function InventoryTurnoverPanel({
  ...
  return (
    ...
    >
      <div className="stripe-metric-grid inventory-summary-grid">
        <div className="pulse-kpi-card inventory-kpi-card">
          <span className="inventory-kpi-label">{t("inventory.stat_stock_cost", language)}</span>
          <div className="inventory-kpi-value-wrap">
            <Typography.Text className="font-tabular inventory-kpi-value">
              {fmtMoney(summary.ending_stock_cost, currency)}
            </Typography.Text>
          </div>
          <Text type="secondary" className="inventory-kpi-detail">
            {t("inventory.stat_stock_qty", language, { qty: summary.ending_stock_qty.toLocaleString() })}
          </Text>
        </div>

        <div className="pulse-kpi-card inventory-kpi-card">
          <span className="inventory-kpi-label">{t("inventory.stat_transit_cost", language)}</span>
          <div className="inventory-kpi-value-wrap">
            <Typography.Text className="font-tabular inventory-kpi-value">
        ...
        <div className="pulse-kpi-card inventory-kpi-card">
          <span className="inventory-kpi-label">{t("inventory.stat_doi", language)}</span>
          <div className="inventory-kpi-value-wrap">
            <Typography.Text
              className={`font-tabular inventory-kpi-value${summary.doi == null ? " is-muted" : summary.doi <= 30 ? " is-positive" : summary.doi <= 60 ? " is-info" : " is-warning"}`}
            >
        ...
        <div className="pulse-kpi-card inventory-kpi-card">
          <span className="inventory-kpi-label">{t("inventory.stat_turnover", language)}</span>
          <div className="inventory-kpi-value-wrap">
            <Typography.Text className="font-tabular inventory-kpi-value">
        ...
        <div className="pulse-kpi-card inventory-kpi-card">
          <span className="inventory-kpi-label">{t("inventory.stat_carrying_cost", language)}</span>
          <div className="inventory-kpi-value-wrap">
            <Typography.Text className="font-tabular inventory-kpi-value is-negative">
